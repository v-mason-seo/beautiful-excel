...
- [ ] xlrd를 이용한 XLS 파일 읽기 (레거시 지원)
- [ ] 파일 대화상자 구현
- [ ] 엑셀 데이터를 그리드에 표시
- [ ] 서식 정보 유지 (폰트, 색상, Bold 등)
...
#### 4.1 폰트/크기 일괄 변환
- [ ] 설정 메뉴의 글자 크기 값 가져오기
- [ ] 모든 데이터 셀에 폰트 크기 일괄 적용
- [ ] 기본 글꼴 설정 (예: Arial, 맑은 고딕)

#### 4.2 빈 셀 최적화
- [ ] 각 컬럼별 빈 셀 비율 계산
- [ ] 빈 셀이 많은 컬럼 식별
- [ ] 해당 컬럼의 헤더 글자 크기 축소 (최소 8pt)
- [ ] 컬럼 너비 최소화 (최소 가독성 유지)

#### 4.3 컬럼별 공통 텍스트 Bold 처리
- [ ] 각 컬럼별 데이터 수집
- [ ] 앞에서부터 공통 문자열 탐지 알고리즘 구현
  ```python
  # 예시: ["서울-강남-대치", "서울-강남-역삼", "서울-서초-방배"]
  # → "서울-" 부분을 Bold 처리
  ```
- [ ] 공통 영역에 Bold 서식 적용
- [ ] Rich Text 또는 HTML 형식으로 표현

#### 4.4 헤더 자동 줄바꿈
- [ ] 각 컬럼의 데이터 최대 길이 계산
- [ ] 헤더 길이와 비교
- [ ] 헤더가 더 긴 경우 자동 줄바꿈 적용
- [ ] 줄바꿈된 헤더의 행 높이 자동 조정
...
#### 5.1 용지 크기 계산
- [ ] A4 규격: 210mm × 297mm (가로) / 297mm × 210mm (세로)
- [ ] A3 규격: 297mm × 420mm (가로) / 420mm × 297mm (세로)
- [ ] 여백 설정 (상하좌우 기본 10mm)
...
#### 6.1 인쇄 미리보기
- [ ] QPrintPreviewDialog 구현
- [ ] 최적화된 레이아웃 미리보기
- [ ] 페이지별 분할 확인

#### 6.2 인쇄 기능
- [ ] QPrinter를 이용한 인쇄 구현
- [ ] 용지 설정 반영 (A4/A3, 가로/세로)
- [ ] 인쇄 대화상자 구현
...
| Phase 3 | 데이터 입출력 기능 | ⏳ 대기 | - |
| Phase 4 | 핵심 최적화 로직 | ⏳ 대기 | - |
| Phase 5 | 용지 규격 맞춤 최적화 | ⏳ 대기 | - |
| Phase 6 | 출력 및 미리보기 | ⏳ 대기 | - |
